  ...
  try {
    ...
    const contextText = criticalSkus
      ? criticalSkus.map((s: any) => 
          `- SKU: ${s.sku_name} (${s.sku_id}) | Cat: ${s.category} | Estoque: ${s.current_stock} (Mínimo Ideal: ${s.reorder_point}) | Score: ${s.score_risco} | Status: ${s.current_stock <= s.reorder_point ? 'BOMBA/FALTA' : 'ALERTA'}`
        ).join('\n')
